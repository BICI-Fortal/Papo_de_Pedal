...
params:
  file_path: 'H:/.shortcut-targets-by-id/15QBAXOC5TdfNgOtQZh2jvqN37NjxkSM0/@BICI/06_Eixo Dados/Projetos/Papo de pedal/Edicao 051224/result/pesquisa_papo_pedal.rds'
  title: 'Edição 05/12/2024'
includes:
  in_header:
    style.css
    actions.js
  ...
  tags$table(
    ...
    tags$tr(
      ...
      tags$td(
        style = "width:35%;",
        tags$span(
            "Baixe os dados dessa edição:",
            tags$br(),
            tags$span(
              tags$a(
                href = paste0("pesquisa_",description$data,".csv"),
                style = "text-decoration:none;",
                download = paste0("pesquisa_",description$data,".csv"),
                tags$span(
                  class = "tip",
                  tags$span(
                    class = "tiptext",
...
if(length(unique(p$Idade_lbl))<4){
  p <- p %>% 
    mutate(Idade_lbl = cut(idade,c(seq(0,80,5),100)))
}
...
p <- plot_ly() %>%
  ...
  layout(
    ...
    xaxis = list(title = "Idade",range = c(0,n_groups+0.5)),
...
<div class="rightcol_top" style="width:70%;flex: 5 1 600px; position:relative;display:flex;">
...
if("qual_e_o_principal_problema_que_voce_enfrenta_ao_pedalar_em_fortaleza_agrupado" %in% names(df)){
  try({
  df <- df %>% 
